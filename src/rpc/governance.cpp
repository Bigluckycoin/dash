...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 HATCH irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on hatchcentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in hatch.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("hatch.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in hatch.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Hatch features */
    { "hatch",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "hatch",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "hatch",               "gobject",                &gobject,                true,  {} },
    { "hatch",               "voteraw",                &voteraw,                true,  {} },
