...
const char * const BITCOIN_CONF_FILENAME = "hatch.conf";
const char * const BITCOIN_PID_FILENAME = "hatchd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "hatch" is a composite category enabling all Hatch-related debug output
                if(ptrCategory->count(std::string("hatch"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\HatchCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\HatchCore
    // Mac: ~/Library/Application Support/HatchCore
    // Unix: ~/.hatchcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HatchCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/HatchCore";
#else
    // Unix
    return pathRet / ".hatchcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty hatch.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override hatch.conf
