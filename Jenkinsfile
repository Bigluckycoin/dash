...
// which are part of the Hatch repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="hatch-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /hatch-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "hatchpay-hatch/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from hatchpay-hatch/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'hatchpay-hatch/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from hatchpay-hatch/develop"
            }
          }

          if (hasCache) {
            sh "cd /hatch-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /hatch-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /hatch-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /hatch-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /hatch-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /hatch-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /hatch-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
